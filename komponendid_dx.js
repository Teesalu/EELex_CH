...
function showDicLogs() {
    ...
    if (!myForward) {
        ...
        //myForward.setAttribute("action", "dicPrint.cgi");
...
function srvXmlValidate() {
    ...
    } else if (scriptName == "art_dx.cgi" || scriptName == "art_dx_new.cgi") { // "sõltumatu" versioon
...
/**
* Kutsutakse välja nupu "Lisa artikkel" all browser variandis.
* 
* @method imgArtAddClick_dx
*/
function imgArtAddClick_dx(sParms) {
    ...
    if (dic_desc == "har" || dic_desc == "has") {
        ...
        setXmlNodeValue(uusAttr, "uus");
        setMyNSNamedItem(oCNode, uusAttr);
    } else if (dic_desc == "kol") {
        uusAttr = createMyNSAttribute(oFrDom, DICPR + ":u", DICURI);
        setXmlNodeValue(uusAttr, "uus");
        setMyNSNamedItem(oCNode, uusAttr);
    } else if (dic_desc == "efi") {
        uusAttr = createMyNSAttribute(oFrDom, DICPR + ":u", DICURI);
        setXmlNodeValue(uusAttr, "uus");
        setMyNSNamedItem(oCNode, uusAttr);
    } else if (dic_desc == "ev2") {
        uusAttr = createMyNSAttribute(oFrDom, DICPR + ":u", DICURI);
        setXmlNodeValue(uusAttr, "uus");
        setMyNSNamedItem(oCNode, uusAttr);
    }
...
function imgArtSaveClick() {
    ...
    //window.alert(jooksevArtStr);
    //window.alert(artOrgString);

    // ATS03 Kas tavaline salvestus või lõpumärgi lisamine
    if (!dEndNote) {
    ...
    //window.alert(getXmlString(oTNode));

    // ATS03 Kas tavaline salvestus või lõpumärgi lisamine
    if (dEndNote) {

        var dRet = atsArtAddDellEndnote(cRezTLPTA);
        dEndNote = false;
        if (!dRet) {
            return;
        }
    ...
    //var nRetBtn = window.confirm(getXmlString(cmdXmlDom));
...
function getSortVal(oMNode, inclHoms, yvOrder) {
    ...
    // Eesti kohanimeraamat (knr)(10.03.2015)
    if (dic_desc == "knr") {
        var nPos = -1;
        var xxVal = nmVal.toLowerCase();
        nPos = xxVal.indexOf("å");
        if (nPos > -1) {
            nmVal = nmVal + "_";
    ...
    nmVal = nmVal.replace(/Đ/g, "D");
    nmVal = nmVal.replace(/đ/g, "d");
    // ukraina tähestikule
    if (dic_desc == "eue" || dic_desc == "ues") {
        nmVal = nmVal.replace(/Cs/g, "Č");
        nmVal = nmVal.replace(/cs/g, "č");
        nmVal = nmVal.replace(/Dzs/g, "Ď");
        nmVal = nmVal.replace(/dzs/g, "ď");
        nmVal = nmVal.replace(/Dz/g, "Ð");
        nmVal = nmVal.replace(/dz/g, "ð");
        nmVal = nmVal.replace(/Gy/g, "Ģ");
        nmVal = nmVal.replace(/gy/g, "ģ");
        nmVal = nmVal.replace(/Ly/g, "Ļ");
        nmVal = nmVal.replace(/ly/g, "ļ");
        nmVal = nmVal.replace(/Ny/g, "Ņ");
        nmVal = nmVal.replace(/ny/g, "ņ");
        nmVal = nmVal.replace(/Sz/g, "Š");
        nmVal = nmVal.replace(/sz/g, "š");
        nmVal = nmVal.replace(/Ty/g, "Ţ");
        nmVal = nmVal.replace(/ty/g, "ţ");
        nmVal = nmVal.replace(/Zs/g, "Ž");
        nmVal = nmVal.replace(/zs/g, "ž");
    }

    // Baltisaksa sõnastik (bss)
    if (dic_desc == "bss") {

        // Lisati 13.01.2020
        // ----------------------------------------
        var nPosxxx = -1;
        nPosxxx = nmVal.indexOf("ß");
        if (nPosxxx > -1) {
            nmVal = nmVal.replace(/ß/g, "ss");
        }
        // ----------------------------------------
        ...
        var xxVal = nmVal.toLowerCase();
        var xxCnt = 0;
        nPos = xxVal.indexOf("ä");
        if (nPos > -1) {
            xxCnt = xxCnt + 1;
        }
        nPos = xxVal.indexOf("ö");
        if (nPos > -1) {
            xxCnt = xxCnt + 1;
        }
        nPos = xxVal.indexOf("ü");
        if (nPos > -1) {
            xxCnt = xxCnt + 1;
        }
        if (xxCnt > 0) {
            nmVal = nmVal + "_";
            nmVal = nmVal.replace(/Ä/g, "A");
            nmVal = nmVal.replace(/ä/g, "a");
            nmVal = nmVal.replace(/Ö/g, "O");
            nmVal = nmVal.replace(/ö/g, "o");
            nmVal = nmVal.replace(/Ü/g, "U");
            nmVal = nmVal.replace(/ü/g, "u");
        }
    }
    ...
    if (dic_desc == "ems" || dic_desc == "vms" || dic_desc == "hms" || dic_desc == "ims" || dic_desc == "kes" || dic_desc == "sas" || dic_desc == "mes") {
    ...
    if (inclHoms && hnr) {
        ...
        if ((msLsp.length > 0) && (nmVal.charAt(nmVal.length - 1) == msLsp)) {
            if (dic_desc == "ev2") {
                nmVal = nmVal.substr(0, nmVal.length - 1) + msLsp + hnr;
            } else {
...
function valmisParingud() {
    ...
    if (dic_desc == "har") {
        ...
        LisaKoondParing("c:A/.//c:m[%s] or .//c:ps[%s]",
            "c:m||" + DICURI + "|0|märksõna ja tema ühendid|XML",
            "Märksõna ja tema ühendid (c:m või c:ps)    <--    'märksõna'",
            "");
        ...
        LisaKoondParing("s:A/.//s:m[%s] or .//s:ps[%s]",
            "s:m||" + DICURI + "|0|märksõna ja tema ühendid|XML",
            "Märksõna ja tema ühendid (s:m või s:ps)    <--    'lemma'",
            "");
    } else if (dic_desc == "ss1") {
        LisaKoondParing("s:A/.//s:m[%s] or .//s:ps[%s]",
            "s:m||" + DICURI + "|0|märksõna ja tema ühendid|XML",
...
/**
* ATS05 Leiame loendite nimekirja
* 
* @method atsLoendid
*/
function atsLoendid() {

    // Loeme file 'xsd/schema_xxx.xsd'.
    var oXSDDom = IDD("File", "xsd/schema_" + dic_desc + ".xsd", false, false, null);

    // Leiame kõik elemendid, millede nime on 'xs:element'.
    var ooElemNodes = oXSDDom.getElementsByTagName('xs:element');
    ...
    if (nnLenght > 0) {
        ...
        for (let i = 0; i < nnLenght; i++) {
            ...
            // Kontrollime kas antud elemendil on attr 'type' (true,flse)
            if (ooENode.hasAttribute("type")) {
                // Attr. 'type' väärtus
                var cAttrTypeValue = ooElemNodes.item(i).attributes[1].value;
                if (cAttrTypeValue.indexOf("tyybid:") != -1) {

                    cAttrTypeValue = cAttrTypeValue.replace("tyybid:", "");
                    cAttrTypeValue = cAttrTypeValue.replace("_tyyp", "");

                    if (ooENode.hasAttribute("name")) {
                        // Attr. 'type' väärtus
                        cAttrNameValue = ooElemNodes.item(i).attributes[0].value;

                        if (cAttrTypeValue != cAttrNameValue) {
                            cAttrNameValue = cAttrTypeValue;
                        }

                    }

                    // Leiame xs:elemendi alt xs:documentation.
                    var ooDocumNodes = ooENode.getElementsByTagName('xs:documentation');
                    var nnDocLenght = ooDocumNodes.length;
                    //window.alert(nnDocLenght);
                    for (let k = 0; k < nnDocLenght; k++) {
                        var ooDNode = ooDocumNodes.item(k);

                        if (ooDNode.hasAttribute("xml:lang")) {

                            var cAttrLangValue = ooDocumNodes.item(k).attributes[0].value;
                            if (cAttrLangValue == "et") {
                                var ccDocValue = ooDNode.childNodes[0].nodeValue;

                                var cTulem = " " + ccDocValue + " (" + cAttrNameValue + ")";
                                //var ccID = "id" + cAttrNameValue;
                                var ccID = cAttrNameValue;
                                var cRetx = "<item id='" + ccID + "' text='" + cTulem + "' img='TableHS.png'/>"

                                aDicArray.push(cRetx);
                                aFindArrayId.push(ccID);
                                aFindArrayNm.push(ccDocValue + " (" + ccID + ")");
                                //window.alert(cRetx); 
                                //window.alert(getXmlString(ooDNode));
                            }
                        }
                    }
                }
                ...
                var ooExteNodes = ooENode.getElementsByTagName('xs:extension');
                //window.alert(ooExteNodes.length); 
                if (ooExteNodes.length > 0) {

                    var ooExNode = ooExteNodes.item(0);
                    if (ooExNode.hasAttribute("base")) {

                        var cAttrBaseValue = ooExteNodes.item(0).attributes[0].value;

                        if (cAttrBaseValue.indexOf("tyybid:") != -1) {
                            cAttrBaseValue = cAttrBaseValue.replace("tyybid:", "");
                            cAttrBaseValue = cAttrBaseValue.replace("_tyyp", "");
                            //var ccExtValue = ooExNode.childNodes[0].nodeValue;

                            if (cAttrBaseValue != cAttrNameValue) {
                                cAttrNameValue = cAttrBaseValue;
                            }
    ...
    // Leiame kõik elemendid, millede nime on 'xs:attribute'.
    ooElemNodes = oXSDDom.getElementsByTagName('xs:attribute');
    // Määrame leitud elementide arvu (integer).
    nnLenght = ooElemNodes.length;
        ...
        for (let i = 0; i < nnLenght; i++) {
            ...
            if (ooENode.hasAttribute("type")) {
                ...
                if (cAttrTypeValue.indexOf("tyybid:") != -1) {
                    ...
                    if (ooENode.hasAttribute("name")) {
                        // Attr. 'type' väärtus
                        var cAttrNameValue = ooElemNodes.item(i).attributes[0].value;
                        if (cAttrTypeValue != cAttrNameValue) {
                            cAttrNameValue = cAttrTypeValue;
                        }
                    }
...
/**
* ATS05 Sõnastiku lukustamine hooldetöödeks
* 
* @method atsKeyYesNo
*/
function atsKeyYesNo() {
    var dOpn, dRet = false;
    if (atsLukk) {
        dRet = window.confirm("Kas soovite avada sõnastiku tööks?");
        dOpn = false;
    } else {
        dRet = window.confirm("Kas soovite sulgeda sõnastiku tööks?");
        dOpn = true;
    }
    if (dRet) {
        var rspDOM;
        var xh = exCGISync("tools.cgi", "exsaSetField" + PD + dic_desc + PD + sUsrName + PD + "shsconfig_" + dic_desc + ".xml" + PD + "msg" + PD + dOpn);
        if ((xh.statusText == "OK")) {
...
/**
* ATS05 Teade sõnastiku lukustamise kohta
* 
* @method atsKeyMessage
*/
function atsKeyMessage() {
    var dRet = atsLukk;
    if (atsLukk) {
        window.alert("Sõnastik on suletud hooldustöödeks. Proovige varsti uuesti!");
    }
    return dRet;
}
...
* 
* @method atsPrintDataSet
* @param {Object} nCntRowID
*/
function atsPrintDataSet(qM, nCntRowID) {
    var nMaxLeid = parseInt(cMaxLeid);
    var nMaxPrint = parseInt(cMaxPrint);
    dMod = true;
    if (nMaxLeid > nMaxPrint) {
        window.alert("Viga max. väljatrüki arvus(shsconfig_" + dic_desc + ".xml)");
        return;
    }

    if (cmdXmlPrint == "") {
        //window.alert("Trükkimiseks on vaja enne teha päring!");
        atsAlert("1", "Teade", "Trükkimiseks on vaja enne teha päring!", "Jah")

    } else {
        //window.confirm("Start...");
        sCmdId = "ClientPrint";
        var cmdXmlDom = IDD("String", cmdXmlPrint, false, false, null);
        StartOperation(cmdXmlDom);
    }
}
...
* ATS05 Trükki mineva andmekogumi ettevalmistamine
* 
* @method atsBrowseDataSet
* @param {Object} nCntRowID
*/
function atsBrowseDataSet(qM, nCntRowID) {
    var nMaxLeid = parseInt(cMaxLeid);
    var nMaxPrint = parseInt(cMaxPrint);
    dMod = false;
    if (nMaxLeid > nMaxPrint) {
...
* ATS05 Trükki mineva andmekogumi ettevalmistamine
* 
* @method atsPrintVahemik
* @param {Object} qm
*/
function atsPrintVahemik(qM) {
    var cVahemik = "";
    var cAVah = "";
    var cLVah = "";
    ...
    while (i == 0) {
        ...
        cVahemik = prompt("Sisestage: algartikli märksõna ja lõppartikli märksõna (näiteks kujul: msõna1 > msõna2). Kahe sõna vahel peab olema märk '>'. ", cVahemik);

        if (cVahemik == null) {
            return
        } else {
            var nMark = cVahemik.indexOf(">");
            if (nMark > -1) {
                var cStarEnd = cVahemik.split(">");
                cAVah = cStarEnd[0]
                cLVah = cStarEnd[1]
                cAVah = cAVah.trim();
                cLVah = cLVah.trim();
    ...
    //window.alert("cAVah =" + cAVah + " cLVah =" + cLVah);

    //return

    //cmdXmlPrint = "<prm><cmd>ClientPrint</cmd><vol>volxxx</vol><nfo>[1. märksõna (x:A/x:P[1]/x:mg[1]/x:m[1]) [ &gt;= 'avxxx' JA &lt;= 'lvxxx' ]], tt-tu, m-deta</nfo><axp>x:A[x:P[1]/x:mg[1]/x:m[1][al_p:compMs(., 'avxxx', 'lvxxx') &gt; 0]]</axp><exp>.//x:m[text()[al_p:srch(.) &gt; 0]]</exp><wC>0</wC><wS>-1</wS><evP>.//x:m/text()</evP><qn>x:m</qn><fSrP>^ka</fSrP><pFakPtrn>()</pFakPtrn><fMsqlP></fMsqlP><hlP>%5Cbka</hlP><qM>XML</qM><pQrySql>SELECT synxxx.md AS md, msid.ms AS l, msid.ms_att_i AS ms_att_i, msid.ms_att_liik AS ms_att_liik, msid.ms_att_ps AS ms_att_ps, msid.ms_att_tyyp AS ms_att_tyyp, msid.ms_att_mliik AS ms_att_mliik, msid.ms_att_k AS ms_att_k, msid.ms_att_mm AS ms_att_mm, msid.ms_att_st AS ms_att_st, msid.ms_att_vm AS ms_att_vm, msid.ms_att_all AS ms_att_all, msid.ms_att_uus AS ms_att_uus, msid.ms_att_zs AS ms_att_zs, msid.ms_att_u AS ms_att_u, msid.ms_att_em AS ms_att_em, synxxx.G AS G, synxxx.art AS art, synxxx.K AS K, synxxx.T AS T, synxxx.TA AS TA, synxxx.PT AS PT, synxxx.vol_nr AS vol_nr FROM msid, synxxx WHERE (synxxx.G = msid.G AND msid.dic_code = 'synxxx' AND (STRCMP(msid.ms_nos, 'avxxx') &gt;= 0 AND STRCMP(msid.ms_nos, 'lvxxx') &lt;= 0) AND synxxx.vol_nr = 1) GROUP BY synxxx.G ORDER BY synxxx.ms_att_OO</pQrySql></prm>";

    cmdXmlPrint = "<prm><cmd>ClientPrint</cmd><vol>volxxx</vol><nfo>[1. märksõna (x:A/x:P[1]/x:mg[1]/x:m[1]) [ &gt;= 'avxxx' JA &lt;= 'lvxxx' ]],";
    cmdXmlPrint = cmdXmlPrint + " tt-tu, m-deta</nfo><axp>x:A[x:P[1]/x:mg[1]/x:m[1][al_p:compMs(., 'avxxx', 'lvxxx') &gt; 0]]</axp><exp>.//x:m[text()";
    cmdXmlPrint = cmdXmlPrint + "[al_p:srch(.) &gt; 0]]</exp><wC>0</wC><wS>-1</wS><evP>.//x:m/text()</evP><qn>x:m</qn><fSrP>^ka</fSrP><pFakPtrn>()</pFakPtrn>";
    cmdXmlPrint = cmdXmlPrint + "<fMsqlP></fMsqlP><hlP>%5Cbka</hlP><qM>XML</qM><pQrySql>SELECT synxxx.md AS md, msid.ms AS l, msid.ms_att_i AS ms_att_i,";
    cmdXmlPrint = cmdXmlPrint + " msid.ms_att_liik AS ms_att_liik, msid.ms_att_ps AS ms_att_ps, msid.ms_att_tyyp AS ms_att_tyyp, msid.ms_att_mliik AS ms_att_mliik,";
    cmdXmlPrint = cmdXmlPrint + " msid.ms_att_k AS ms_att_k, msid.ms_att_mm AS ms_att_mm, msid.ms_att_st AS ms_att_st, msid.ms_att_vm AS ms_att_vm,";
    cmdXmlPrint = cmdXmlPrint + " msid.ms_att_all AS ms_att_all, msid.ms_att_uus AS ms_att_uus, msid.ms_att_zs AS ms_att_zs, msid.ms_att_u AS ms_att_u,";
    cmdXmlPrint = cmdXmlPrint + " msid.ms_att_em AS ms_att_em, synxxx.G AS G, synxxx.art AS art, synxxx.K AS K, synxxx.T AS T, synxxx.TA AS TA, synxxx.PT AS PT,";
    cmdXmlPrint = cmdXmlPrint + " synxxx.vol_nr AS vol_nr FROM msid, synxxx WHERE (synxxx.G = msid.G AND msid.dic_code = 'synxxx' AND (STRCMP(msid.ms_nos, 'avxxx') &gt;= 0";
    cmdXmlPrint = cmdXmlPrint + " AND STRCMP(msid.ms_nos, 'lvxxx') &lt;= 0) AND synxxx.vol_nr = 1) GROUP BY synxxx.G ORDER BY synxxx.ms_att_OO</pQrySql></prm>";


    //window.alert(dic_desc);


    cmdXmlPrint = cmdXmlPrint.replace(/avxxx/g, cAVah);
    cmdXmlPrint = cmdXmlPrint.replace(/lvxxx/g, cLVah);

    cmdXmlPrint = cmdXmlPrint.replace(/volxxx/g, currentVolId);
    cmdXmlPrint = cmdXmlPrint.replace(/synxxx/g, dic_desc);

    //window.alert(cmdXmlPrint);
...
/**
* Loob uues aknas prindi faili.
* 
* @method vaatedRefreshPrint1
*/
function vaatedRefreshPrint1(vastus) {
    var oclid, newFragment;
    newFragment = valaXslProse.transformToFragment(vastus, document);

    //window.alert(getXmlString(newFragment));

    if (newFragment) {
        var myWindow = window.open("", "Print", "scrollbars=yes,resizable=yes,top=100,left=100,width=900,height=700");
        var xcss = "css/gendView_" + dic_desc + ".css";
        myWindow.document.write("<!DOCTYPE HTML><html><head><link id='valaCss' rel='stylesheet' type='text/css' href='" + xcss + "' /><title>Print(" + dic_desc + ")</title></head><body><div id='printDiv' style='width:100%;height:100%;'></div></body></html>");
        myWindow.printDiv.appendChild(newFragment);
    }
} // vaatedRefreshPrint
...
* Loob uues aknas prindi faili.
* 
* @method vaatedRefreshPrint
*/
function vaatedRefreshPrint(vastus) {
    var oclid, newFragment;
    newFragment = valaXslProse.transformToDocument(vastus, document);
    ...
    if (dMod == true) {
        if (newFragment) {
            //var myWindow = window.open("", "Print", "scrollbars=yes,resizable=yes,top=100,left=100,width=900,height=700");
            let zTxt1 = "Kui soovite kuvada trükitud artikleid Wordis, siis peate tegema trükitud tekstiga akna aktiivseks(kui te ei ole antud lehel!) ja kopeerima kogu teksti või osa tekstist, mida on vaja Wordi viia (ctrl + A ja copy). Seejärel käivitage Word ja tehke tühjale lehele paste kopeeritud tekstist." ;
            zTxt1 = zTxt1 + " Soovite aga, trükiks formeeritud teksti kohe printerile trükkida, siis klõpsake antud tekstile hiire parempoolse klahviga, mille tulemusena avaneb menüü ja sealt valige 'Print'.";
            ...
            cNewHtml = xslJavaScript(cNewHtml);
 
            cNewHtml = "<title>Trükk (" + dic_desc + ")</title>" + cNewHtml

            myWindow.document.write(cNewHtml);
 
  
        }
    } else {
        dMod = true;
        atBrowsey(cNewHtml,1200,1000);
    }
...
* Loob uues aknas prindi faili.
* 
* @method vaatedRefreshBrowse
*/
function vaatedRefreshBrowse(vastus) {
    var oclid, newFragment;
