...
function SetEdits() {
    ...
    if (mT == "") {
        if (snQName == "n:aa") { // Eesti kohanimeraamat: kirjandus; teos: autor aasta;
            ...
            mT = "pikkLoend";
        } else if (snQName == "x:aa") { // ets: standard
            if (dic_desc != 'ysl') {
               mT = "pikkLoend";
            }
        } else if (snQName == "c:vbf") { // põhisõnavara sõnastik; viipe fail;
            mT = "flashPlayer";
        } else if (snQName == "x:vbf") { // vkq: viipe fail;
            mT = "flashPlayer";
        } else if (snQName == "x:vbdf") { // vkq: definitsiooni viipe fail;
            mT = "flashPlayer";
        } else if (snQName == "x:vbnf") { // vkq: näitelause viipe fail;
            mT = "flashPlayer";
        } else if (snQName == "c:plf") { // põhisõnavara sõnastik;<pilt>
            mT = "flashPlayerImage";
        } else if (snQName == "x:plf") { // seto sõnastik;<pilt>
            mT = "flashPlayerImage";
        } else if (snQName == "d:pf") { // Saksa-Liivi;<pilt>
            mT = "flashPlayerImage"; 
    ...
    if (mT == "pikkLoend") {
        ...
        retVal = window.showModalDialog("html/listingLoend.htm?m=14", smdArgs, "dialogHeight:500px;dialogWidth:1024px;center:yes;edge:sunken;help:no;resizable:yes;scroll:yes;status:no;unadorned:no");
        oClicked.style.border = oClickedBorder;
...
function vbSetKBLang(l) {
    ...
    try {
        ...
        if (sta < -1) {
            ...
            msgSisu = "<img src='graphics/error.png' width='16'><b>setKeyboardLayout:</b> " + sta;
             ...
             if (useViitKontr){
                if (atsOnViit(selectedNode.nodeName)) {
			        ...
			        if (ccField != currTekst) {
				       ...
				       ccField = currTekst;
                       selectedNode.text = "Viidete otsimine. Palun oota...";
...
*  
*
* @method kolUpdateColloc
* @param {nsv} uus tekst
* @param {text} tekst
*/
function kolUpdateColloc(nsv) {
//colloc = mse+' '+col
//colloc = col+' '+msj
  if(snQName == "x:mse" ){
      if(selectedNode.nextSibling != null && selectedNode.nextSibling.tagName == "x:col"){
        kolAddColloc(selectedNode.parentNode,nsv+" "+selectedNode.nextSibling.text);
      }
  }
  if(snQName == "x:col" ){
      if(selectedNode.previousSibling != null && selectedNode.previousSibling.tagName == "x:mse"){
        kolAddColloc(selectedNode.parentNode,selectedNode.previousSibling.text+" "+nsv);
      }else if(selectedNode.nextSibling != null && selectedNode.nextSibling.tagName == "x:msj"){
        kolAddColloc(selectedNode.parentNode,nsv+" "+selectedNode.nextSibling.text);
      }
  }
  if(snQName == "x:msj"){
    if(selectedNode.previousSibling != null && selectedNode.previousSibling.tagName == "x:col"){
      kolAddColloc(selectedNode.parentNode,selectedNode.previousSibling.text+" "+nsv);
    }
  }
...
*
* @method kolAddColloc
* @param {node} node kuhu colloc lisada
* @param {text} tekst
*/
function kolAddColloc(node,text) {
if(node.firstChild.tagName == "x:colloc"){
  node.firstChild.text=text;
}else{
  //var ce = node.ownerDocument.createElement("x:colloc");
  //var insertedElement = node.insertBefore(ce, node.firstChild);
  //insertedElement.text=text;
  alert("Peaks looma uue x:colloc: "+text);
}
        ...
        case "string":
            if ((doSpellCheck)) {
                ...
                try {
                    ...
                    if ((sta == 0)) {
                        msgSisu = "<img src='graphics/warning.png' width='16'> " + CHECK_SPELLING + "<b>" + chkvalue + "</b>, " + LANGUAGE_WORD + "'" + elemlang + "' (" + nameLocal + ")!";
                        spn_msg.innerHTML = msgSisu;
                    } else if ((sta == 1)) {
                        spn_msg.innerHTML = '';
                    } else {
                        ...
                        msgSisu = "<img src='graphics/error.png' width='16'><b>mySpellCheck:</b> " + sta;
                        if ((sta == -3)) {
...
function XMLChanged() {
    ...
    if ((oEditDOMRoot.hasChildNodes())) {
        ...
        if ((urindex == 0)) {
            img_Undo.src = "graphics/back_grey 16-16.png";
        } else {
            img_Undo.src = "graphics/back 16-16.png";
        }
        if ((urindex == urlast)) {
            img_Redo.src = "graphics/forward_grey 16-16.png";
        } else {
            img_Redo.src = "graphics/forward 16-16.png";
        }
...
function lisaVeelArtToolse() {
    ...
    if (dic_desc == "har" || dic_desc == "geo" || dic_desc == "has") {
        //--------------------------------------------------------------------------------
...
function SetSelectedInfo(sNewId, sNewAttrValue) {
    ...
    if (div_ElemEnumsMenu.innerHTML != "") {
        img_ElemEnumsMenu.src = "graphics/downarrow 16-16.png";
        img_ElemEnumsMenu.disabled = false;
    } else {
        img_ElemEnumsMenu.src = "graphics/downarrow_grey 16-16.png";
    ...
    if (div_AttrEnumsMenu.innerHTML != "") {
        img_AttrEnumsMenu.src = "graphics/downarrow 16-16.png";
        img_AttrEnumsMenu.disabled = false;
    } else {
        img_AttrEnumsMenu.src = "graphics/downarrow_grey 16-16.png";
