...
function bodyOnLoad() {
  ...
  // ATS 10.10.2014
  // Viitekontroll
  cfgElem = oConfigDOM.documentElement.selectSingleNode("vcont");
  if (cfgElem) {
    useViitKontr = jsStringToBoolean(cfgElem.text);
  } else {
    useViitKontr = false;
  }
  //
...
function PaneEkspressioonid() {
  ...
  img_ArtDelete.setExpression("src", "(sFromVolume == sDeldVolId) ? 'graphics/backtolist 16-16.ico' : 'graphics/delart 16-16.png'", "JScript")
  ...
  if (sNodeTest == artRada) {//A
    ...
    if (inp_UseGlobal.checked) {
      ...
      if (dic_desc == 'ss1' && seldQn == qn_ms) {
        ...
        elemRada = ".//*[self::s:m or self::s:tul or self::s:mm or self::s:rv or self::s:gen]";
      }
      evPath = elemRada;
    } else {
  ...
  if (jsTrim(ft) == "=.") {
    ...
    if (aElemInfo[3] == "1") {
      if (inp_UseGlobal.checked && dic_desc == 'ss1' && seldQn == qn_ms) {
        ...
        pQrySql = getSqlQuery("s:m;s:tul;s:mm;s:rv;s:gen", ft, hasSeldText, withSymbols, withCase, evPath, elemRada, mySqlAttCond);
      } else {
        pQrySql = getSqlQuery(seldQn, ft, hasSeldText, withSymbols, withCase, evPath, elemRada, mySqlAttCond);
      }
    }
...
// 11.05.2016 ATeesalu
 qryMethod = "MySql";
  ...
  if (dic_desc == "ssx") {
    ...
    if (window.event.srcElement.id == "inp_RunQuery") {
       qryMethod = "XML";
    }
 } else {
...
function koostamiseLopp() {
 ...
 // ATS+++ 30.09.2015
 // ======================================================================================================
 if (dic_desc == 'psv' || dic_desc == 'sto' || dic_desc == 'kes' || dic_desc == 'efi' || dic_desc == 'rks' || dic_desc == 'asp' || dic_desc == 'ev2' || dic_desc == 'teo') {
...
function remKoostamiseLopp() {
 var tlElem = oEditDOMRoot.firstChild.selectSingleNode(DICPR + ":KL");
 // ======================================================================================================
 // ATS+++ 30.09.2015
 // ======================================================================================================
 if (tlElem) {
...
function completeEntry() {
 ...
 // ATS+++ 30.09.2015
 // ======================================================================================================
 if (dic_desc == 'psv' || dic_desc == 'sto'||dic_desc == "kol" || dic_desc == 'kes' || dic_desc == 'efi' || dic_desc == 'rks' || dic_desc == 'asp' || dic_desc == 'ev2' || dic_desc == 'teo') {
  var TLTekst = itmElem.text;
  if (TLTekst.length > 0) {
   var f = false;
   var TLd = TLTekst.split('; ');
...
function ParseSrvInfo(oRespDom) {
 ...
 // ATSoft 13.01.2015 14:58
 try {
 var ccVolx,ccGx,ccMdtx,ccLtx,ccAttrHom,ccNewMdt,ccPth
 var ooArtElements = oRespDom.documentElement.selectNodes("..//A");
 var nnArtElemCnt = ooArtElements.length;
 var ccDicpr = atsDicpr(dic_desc);
 //alert(ccDicpr);
 //alert("ParseSrvInfo");
 //alert(nnArtElemCnt);
 //alert(dic_desc);
  for (i = 0; i < nnArtElemCnt; i ++)
  {
    ccVolx    = ooArtElements(i).selectSingleNode("vf").text;
    ccGx      = ooArtElements(i).selectSingleNode("G").text;
    ccMdtx    = ooArtElements(i).selectSingleNode("md/t").text;
    ...
    ccLtx     = ooArtElements(i).selectSingleNode(ccPth).text;
    ccAttrHom = ooArtElements(i).selectSingleNode(ccPth).getAttribute("i");
    if (ccMdtx==""){
       if (typeof ccAttrHom == "object"){
          ccNewMdt = ccLtx + "*";
          ooArtElements(i).selectSingleNode("md/t").text = ccNewMdt;
       }else{
       //alert("i=" + ccAttrHom + " t=" + ccLtx)
       ccNewMdt = ccLtx + " [▫i='" + ccAttrHom + "']*";
       ooArtElements(i).selectSingleNode("md/t").text = ccNewMdt;
       }
    }else{
       //alert(" t=" + ccMdtx)
   ...
   //alert("AT_ERR: Otsingutulemuste formeerimine vigane!!!")
}
...
function FillArtFrames(oArtNode, delURHistory) {
  ...
  if (tlElem) {
    tlTekst = tlElem.text;
    if (dic_desc != 'psv' && dic_desc != "kol" && dic_desc != "kes" && dic_desc != "sto" && dic_desc != "efi" && dic_desc != "rks" && dic_desc != "asp" && dic_desc != "ev2" && dic_desc != "teo") {
      if (t.length > 0) {
        tlTekst = t + ", " + tlTekst;
      }
    }
  ...
  var msgTekt = "";
 if (k) {
    msgTekt = jsReplace(ENTRY_STARTED, "[%s]", k);
    if (msgTekt.length > 120) {
        msgTekt = msgTekt.substr(0,120) + " {...}";
    }
    msgSisu = "<img src='graphics/msgbox_info_16_16.ico'><span style='color:green;'> " + msgTekt + "</span>";

  }
  if (tlTekst) {

    msgTekt = " Artiklile on lisatud toimetamise lõpu märge: " + tlTekst;
    if (msgTekt.length > 120) {
      msgTekt = msgTekt.substr(0,120) + " {...}";
    }
    msgSisu = "<img src='graphics/msgbox_info_16_16.ico'><span style='color:blue;'>" + msgTekt + "</span>";
  ...
  if ((pt != "")) {
    ...
    if ((srTegija)) {
      if (dic_desc == "psv"||dic_desc == "ems"||dic_desc == "kol") {
        bArtSaveAllowed = true;
        bArtDelAllowed = true;
      }
      else {
...
function ShowDicHelp() {
  if ((appDesc == "EXSA")) {
    ...
    //location.href = "http://eelex.eki.ee/juhendid/help2012/edoc.html";
    window.open("http://eelex.eki.ee/juhendid/help2012/edoc.html", "_blank", "channelmode=no,directories=no,fullscreen=no,location=yes,menubar=no,resizable=yes,scrollbars=yes,status=yes,titlebar=yes,toolbar=yes");
  }
...
function ClickSrToolsMenu() {
  ...
  if ((jsLeft(sCFP, 6) == "scroll")) {
    ...
    if (window.event.srcElement.parentElement.value == "idArtImport") {
      ...
      window.open("html/" + hulgiNimi + ".htm?a=10", "_blank", "width=1024,height=700,channelmode=no,directories=no,fullscreen=no,location=no,menubar=no,resizable=yes,scrollbars=yes,status=yes,titlebar=yes,toolbar=no");
      ...
      retVal = window.showModalDialog("html/getsimpletype.htm?a=65", smdArgs, "dialogHeight:608px;dialogWidth:1024px;center:yes;edge:sunken;help:no;resizable:yes;scroll:yes;status:yes;unadorned:no");
      if (retVal == "updated") {
  ...
  if (retVal) {
    ...
    switch (tarr[0]) {
      ...
      case "eelex_setup":
        ...
        // ATS 10.10.2014
        useViitKontr = jsStringToBoolean(tarr[4]);
        //
